...
Examples:
  ...
  content: "**What**: Wrapped each search term in quotes before passing to FTS5 MATCH\n**Why**: Users typing queries like 'fix auth bug' would crash because FTS5 interprets special chars as operators\n**Where**: internal/store/store.go — sanitizeFTS() function\n**Learned**: FTS5 MATCH syntax is NOT the same as LIKE — always sanitize user input"`),
				...
				mcp.WithString("project",
					mcp.Description("Project identifier. Use the git remote origin normalized to 'host/owner/repo' format (e.g. 'github.com/vertigo7x/postgram'). Run: git remote get-url origin, then strip https://, http://, git@, colon separator, and .git suffix. If no git remote exists, use the directory basename (e.g. 'my-app') and set scope to 'personal'."),
				),
				mcp.WithString("scope",
	...
	if shouldRegister("mem_save_prompt", allowlist) {
		srv.AddTool(
			mcp.NewTool("mem_save_prompt",
				...
				mcp.WithString("project",
					mcp.Description("Project identifier. Use the git remote origin normalized to 'host/owner/repo' format (e.g. 'github.com/vertigo7x/postgram'). If no git remote exists, use the directory basename (e.g. 'my-app')."),
				),
			),
			handleSavePrompt(s),
		)
	...
	if shouldRegister("mem_context", allowlist) {
		srv.AddTool(
			mcp.NewTool("mem_context",
				...
				mcp.WithOpenWorldHintAnnotation(false),
				mcp.WithString("project",
					mcp.Description("Filter by project identifier in 'host/owner/repo' format (e.g. 'github.com/vertigo7x/postgram') or directory basename (e.g. 'my-app'). Omit to retrieve context from all projects."),
				),
				mcp.WithString("scope",
	...
	if shouldRegister("mem_session_start", allowlist) {
		srv.AddTool(
			mcp.NewTool("mem_session_start",
				...
				mcp.WithString("project",
					mcp.Required(),
					mcp.Description("Normalized project identifier. Use the git remote origin in 'host/owner/repo' format (e.g. 'github.com/vertigo7x/postgram'). Run: git remote get-url origin, then strip https://, http://, git@, colon separator, and .git suffix. If no git remote exists, use the directory basename (e.g. 'my-app') and set scope to 'personal'."),
				),
				mcp.WithString("directory",
					mcp.Description("Full local path of the working directory (e.g. '/home/alice/projects/postgram'). Stored for audit purposes; use 'project' for the normalized identifier."),
				),
			),
